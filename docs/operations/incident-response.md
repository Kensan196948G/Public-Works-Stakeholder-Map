...
| 項目 | 内容 |
|---|---|
| 一次対応責任者 | 運用責任者: kensan1969@gmail.com（電話連絡先は運用台帳に別途管理） |
| エスカレーション | 一次 → 技術担当（開発者）→ Cloudflare サポート / ホスト OS 管理者（slo-alerts.md §3） |
| 連絡手段 | GitHub Issue（内部記録）・メール（関係者）・必要に応じサービス提供元 |
|---|---|
...
ローカル PostgreSQL（`127.0.0.1:5432`・db `pwsm`）の接続不可・整合性異常・性能劣化。

| ✅ | 一次対応 |
|---|---|
| ☐ | read-only / メンテナンス表示に切替え、書込みを止める（設計 §18.3） |
| ☐ | 接続失敗か整合性異常かを切り分ける（§6 の DB 接続失敗も参照） |
| ☐ | 復旧手順を実施（軽微: 再接続 / 重度: `rollback.md §3` のダンプ復元） |
...
| ✅ | 初動 |
|---|---|
| ☐ | systemd ログ（`journalctl -u pwsm-api`）・Cloudflare Tunnel ログ・エラー率を確認し、直近デプロイとの相関を見る |
| ☐ | 直近デプロイが原因なら `rollback.md §1`（git checkout + build + restart）を実施 |
...
API からローカル PostgreSQL への接続が失敗する（認証・ネットワーク・リソース上限等）。

| ✅ | 初動 |
|---|---|
| ☐ | `DATABASE_URL`（`apps/api/.env`・Git 管理外）の設定・認証を確認（実値は見ない・出さない） |
| ☐ | PostgreSQL サービス（`systemctl status postgresql`・`pg_isready`）の稼働状態を確認 |
| ☐ | 一時的なら再試行、認証情報の問題なら再設定（人間承認・§3 と連携） |
| ☐ | DB 断中は `/api/v1/health/ready` が **503 `unavailable`** を返す（本番は DB 到達を確認する実装）。これを異常検知の指標とし、fixture へ切替えて ready を成功に見せかけない |
| ☐ | `/api/v1/health/live` は DB を見ずプロセス確認のみで 200 を返す。live=200／ready=503 の乖離で DB 起因と判断 |
